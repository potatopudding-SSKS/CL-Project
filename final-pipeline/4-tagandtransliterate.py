...
def read_data(file_path, has_labels=False):
    """
    Read data from input file.
    Args:
        file_path: Path to input file
        has_labels: Whether the input file has labels (default: False)
    Returns:
...
# Function to process tagged output with transliteration
def process_tagged_output(input_csv, transliteration_engine, output_txt="output.txt"):
                ...
                for word, label in zip(words, labels):
                    if label == 1:  # Transliterated Tamil
                        try:
                            ...
                            elif isinstance(result, dict) and "ta" in result:
                                transliterated = result["ta"][0]
                            else:
                                transliterated = word
...
def main():
    ...
    parser.add_argument(
        "--input", default="corpus.txt", help="Input text file with comments"
    )
    parser.add_argument(
        "--model", default="model/crf_model.pkl", help="Path to trained CRF model"
    )
    parser.add_argument(
        "--output",
        default="crf-tagged/crf-tagged.csv",
